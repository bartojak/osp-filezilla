...
void CControlSocket::LogTransferResultMessage(int nErrorCode, CFileTransferOpData *pData)
{
	...
	else
	{
		...
		else if (nErrorCode == FZ_REPLY_OK && pData->transferInitiated)
			LogMessage(Status, _("File transfer successful"));
		else if (nErrorCode == FZ_REPLY_SKIPPED)
		...
		case cmd_transfer:
			{
				...
				if (!pData->download && pData->transferInitiated)
				{
					...
					else
					{
						...
						bool updated = cache.UpdateFile(*m_pCurrentServer, pData->remotePath, pData->remoteFile, true, CDirectoryCache::file, (nErrorCode == FZ_REPLY_OK || nErrorCode == FZ_REPLY_SKIPPED) ? pData->localFileSize : -1);
...
bool CControlSocket::SetFileExistsAction(CFileExistsNotification *pFileExistsNotification)
{
	...
	case CFileExistsNotification::overwriteNewer:
		...
		else
		{
			...
			ResetOperation(FZ_REPLY_SKIPPED);
		}
		break;
